      ...
      {stats.map((s, i) => (
        ...
        >
          <div className="flex items-center justify-between mb-2">
            ...
            {s.iconName && (
              <div className={`w-6 h-6 rounded-md flex items-center justify-center ${s.accent ? "bg-indigo-50 text-indigo-600" : "bg-slate-50 text-slate-500"}`}>
                <Icon name={s.iconName} className="w-3.5 h-3.5" strokeWidth={2} />
              </div>
            )}
          </div>
          <div className={`text-2xl font-semibold tabular leading-tight ${s.accent ? "text-indigo-700" : "text-text"} ${
